...
>(cache(), errorLogger, async ({ params: { name, year = 0 } = {} }) => {
  ...
  const [group, okrList, reportList] = await Promise.all([
    new PersonnelModel().getGroup({ department: name, passed: true }, ['position'], +year),
    new OKRModel().getAll({ department: name, year }),
    new ReportModel().getAll({ department: name, summary: year }),
  ]);

  return { props: JSON.parse(JSON.stringify({ department, group, okrList, reportList })) };
});
