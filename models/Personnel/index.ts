...
export class PersonnelModel extends BiDataTable<Personnel>() {
  ...
  async getGroup(
    ...
    try {
      return (this.group = groupBy(await this.getAll(filter), data => {
        for (const key of groupKeys) if (data[key] != null) return data[key] as string;
  ...
  async getYearGroup(filter: Partial<NewData<Personnel>>, groupKeys: (keyof Personnel)[]) {
    return (this.group = groupBy(await this.getAll(filter), data => {
      for (const key of groupKeys)
        if (data[key] != null) return new Date(data[key] as TimeData).getFullYear();
