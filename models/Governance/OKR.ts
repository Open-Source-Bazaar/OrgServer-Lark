...
export class OKRModel extends BiDataTable<OKR>() {
  ...
  makeFilter({ year, ...filter }: Partial<NewData<OKR>>) {
    return [`CurrentValue.[year]=${year}`, !isEmpty(filter) && makeSimpleFilter(filter)]
      .filter(Boolean)
      .join('&&');
  }
  ...
  extractFields({
    ...
    return {
      ...
      firstResult: firstResult && normalizeTextArray(firstResult as TableCellText[]),
      secondResult: secondResult && normalizeTextArray(secondResult as TableCellText[]),
      thirdResult: thirdResult && normalizeTextArray(thirdResult as TableCellText[]),
